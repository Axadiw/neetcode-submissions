...
class LRUCache:
    ...
    def get(self, key: int) -> int:
        print(f"get key {key}. nodes_map: {self.nodes_map}")
    ...
    def put(self, key: int, value: int) -> None:
        ...
        print(f"self.length {self.length}, self.max_len: {self.max_len}")
        if self.length < self.max_len:
            print('NIE usuwamy 1 node')
            if self.length == 0:
                self.head = new_node
            ...
            print(f'usuwamy 1 node self.head = {self.head}')
